...
INSERT INTO document_reference (
    uuid, master_identifier, status, type_concept_id, doc_status,
...
SELECT
    ...
    'CURRENT',
    o.concept_id,
    CASE WHEN o.voided = 1 THEN 'ENTEREDINERROR' ELSE 'FINAL' END,
    MAX(om.obs_datetime),
    MIN(ep.provider_id),
    SUBSTRING(COALESCE((SELECT om2.comments FROM obs om2 WHERE om2.obs_group_id = o.obs_id AND om2.comments IS NOT NULL AND om2.voided = 0 ORDER BY om2.obs_id ASC LIMIT 1), ''), 1, 255),
    o.encounter_id,
    p.patient_id,
    o.creator,
    MAX(om.date_created),
...
GROUP BY o.obs_id, o.person_id, o.encounter_id, o.concept_id, o.creator, o.voided, p.patient_id;
...
SELECT
    ...
    CASE
        WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(om.value_text, '?', 1), '.', -1)) = 'pdf' THEN 'application/pdf'
        WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(om.value_text, '?', 1), '.', -1)) IN ('jpg', 'jpeg') THEN 'image/jpeg'
        WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(om.value_text, '?', 1), '.', -1)) = 'png' THEN 'image/png'
        WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(om.value_text, '?', 1), '.', -1)) = 'gif' THEN 'image/gif'
        WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(om.value_text, '?', 1), '.', -1)) = 'bmp' THEN 'image/bmp'
        WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(om.value_text, '?', 1), '.', -1)) = 'webp' THEN 'image/webp'
        WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(om.value_text, '?', 1), '.', -1)) = 'avif' THEN 'image/avif'
        WHEN LOWER(SUBSTRING_INDEX(SUBSTRING_INDEX(om.value_text, '?', 1), '.', -1)) = 'mp4' THEN 'video/mp4'
        ELSE 'application/octet-stream'
    END,
    UUID(),
    om.creator,
    om.date_created,
    COALESCE(om.voided, 0)
FROM obs o
    INNER JOIN obs om ON om.obs_group_id = o.obs_id
    INNER JOIN encounter e ON o.encounter_id = e.encounter_id
    INNER JOIN encounter_type et ON e.encounter_type = et.encounter_type_id
