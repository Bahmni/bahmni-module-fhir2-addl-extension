...
public class BahmniEncounterTranslatorImpl extends EncounterTranslatorImpl {
	...
	public Encounter toOpenmrsType(@Nonnull Encounter existingEncounter, @Nonnull org.hl7.fhir.r4.model.Encounter encounter) {
		...
		EncounterType encounterType = getEncounterTypeTranslator().toOpenmrsType(encounter.getType());
		if (encounterType != null) {
			existingEncounter.setEncounterType(encounterType);
		}
		
		existingEncounter.setPatient(getPatientReferenceTranslator().toOpenmrsType(encounter.getSubject()));
		existingEncounter.setLocation(getEncounterLocationTranslator().toOpenmrsType(encounter.getLocationFirstRep()));
		existingEncounter.setVisit(getVisitReferenceTranlator().toOpenmrsType(encounter.getPartOf()));
		setEncounterProviders(existingEncounter, encounter);
		getEncounterPeriodTranslator().toOpenmrsType(existingEncounter, encounter.getPeriod());
	...
	private void setEncounterProviders(Encounter existingEncounter, org.hl7.fhir.r4.model.Encounter encounter) {
		...
		Set<EncounterProvider> updatedProviders = encounter.getParticipant().stream().map(p -> {
			EncounterProvider ep = getParticipantTranslator().toOpenmrsType(new EncounterProvider(), p);
